...
namespace Movies.APP.Features.Directors
{
    ...
    public class DirectorUpdateHandler : Service<Director>,
        IRequestHandler<DirectorUpdateRequest, CommandResponse>
    {
        public DirectorUpdateHandler(MoviesDb db) : base(db) {}

        public async Task<CommandResponse> Handle(DirectorUpdateRequest request,
            CancellationToken cancellationToken)
        {
            if (await Query().AnyAsync(d =>
                    ...
                    cancellationToken))
            {
                return Error("Another director with the same name exists!");
            }
